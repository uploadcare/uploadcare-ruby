...
module Uploadcare
  class UploaderClient < UploadClient
    ...
    def upload(file:, request_options: {}, **)
      upload_many(files: [file], request_options: request_options, **)
    end
    ...
    def handle_polling_response(response)
      ...
      when 'error'
        raise Exception::RequestError, response['error']
      when 'progress', 'waiting', 'unknown'
        raise Exception::RetryError, response['error'] || 'Upload is taking longer than expected. Try increasing the max_request_tries config if you know your file uploads will take more time.' # rubocop:disable Layout/LineLength
      end
