    ...
    def call
      ExampleHelper.run do |client|
        ...
        when 'put_files_uuid_storage.rb'
          ExampleHelper.with_uploaded_file(store: false) do |file|
            file.store
          end
        ...
        when 'put_files_storage.rb'
          ExampleHelper.with_uploaded_files(store: false) do |files|
        ...
        when 'post_files_remote_copy.rb'
          target = ENV['UPLOADCARE_REMOTE_STORAGE']
          ExampleHelper.skip('Set UPLOADCARE_REMOTE_STORAGE to a configured custom storage name.') if target.to_s.empty?
          ExampleHelper.with_uploaded_file do |file|
        ...
        when 'post_webhooks.rb'
          ExampleHelper.with_webhook do |webhook|
            {
              'id' => webhook.id,
