...
**API Overview**
This README focuses on common workflows. For the full API reference, use the generated YARD docs (`mise exec -- bundle exec yard doc`) or browse `api_examples/` and `examples/` in this repo.

**Core Entry Points**
- `Uploadcare.configure { |config| ... }` and `Uploadcare::Configuration.new(**options)` for configuration.
- `Uploadcare::UploadClient` and `Uploadcare::RestClient` for direct API access.
- `Uploadcare::Uploader` for smart uploads and high-level helpers.
- Resource classes like `Uploadcare::File`, `Uploadcare::Group`, and `Uploadcare::Webhook` for REST workflows.
