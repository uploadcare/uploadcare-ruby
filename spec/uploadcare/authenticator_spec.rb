...
RSpec.describe Uploadcare::Authenticator do
  ...
  let(:authenticator) { described_class.new(config: config) }
  let(:http_method) { 'GET' }
  ...
  describe '#headers' do
    ...
    context 'when using Uploadcare auth' do
      ...
      context 'when public_key is blank' do
        let(:public_key) { '' }

        it 'raises AuthError' do
          expect do
            authenticator.headers(http_method, uri, body)
          end.to raise_error(Uploadcare::Exception::AuthError, 'Public Key is blank.')
        end
      end
    end
  end
