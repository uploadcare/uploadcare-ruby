...
module Uploadcare
  RSpec.describe WebhookSignatureVerifier do
    ...
    describe '.valid?' do
      ...
      context 'when signing_secret is missing and UC_SIGNING_SECRET env var is set' do
        before { ENV['UC_SIGNING_SECRET'] = '12345X' }
        after { ENV.delete('UC_SIGNING_SECRET') }

        let(:params) { super().except(:signing_secret) }

        it 'uses environment variable and returns true' do
          expect(signature_valid?).to be_truthy
        end
      end

      context 'with invalid parameters' do
        let(:params) { { signing_secret: 'secret', x_uc_signature_header: 'invalid', webhook_body: 'body' } }
    ...
    # v4.4.3 compatibility test
    describe 'Uploadcare::Param::WebhookSignatureVerifier' do
      it 'is accessible via Param namespace for v4.4.3 compatibility' do
        expect(Uploadcare::Param::WebhookSignatureVerifier).to eq(described_class)
      end

      it 'works the same way via Param namespace' do
        result1 = described_class.valid?(params)
        result2 = Uploadcare::Param::WebhookSignatureVerifier.valid?(params)
