...
RSpec.describe Uploadcare::DocumentConverter do
  ...
  describe '#info' do
    ...
    subject { document_converter.info(uuid: uuid) }

    before do
      allow_any_instance_of(Uploadcare::DocumentConverterClient).to receive(:info)
  ...
  describe '.convert_document' do
    ...
    subject { described_class.convert_document(params: document_params, options: options) }

    before do
      allow_any_instance_of(Uploadcare::DocumentConverterClient).to receive(:convert_document)
        .with(paths: ['doc_uuid/document/-/format/pdf/'], options: options, request_options: {})
        .and_return(response_body)
    end
  ...
  describe '#status' do
    ...
    subject { document_converter.fetch_status(token: token) }

    before do
      allow_any_instance_of(Uploadcare::DocumentConverterClient).to receive(:status)
