...
RSpec.describe Uploadcare::VideoConverter do
  ...
  describe '.convert' do
    ...
    before do
      allow_any_instance_of(Uploadcare::VideoConverterClient).to receive(:convert_video)
        .with(paths: ['video_uuid/video/-/format/mp4/-/quality/lighter/'], options: options, request_options: {})
  ...
  describe '#fetch_status' do
    ...
    subject { video_converter.fetch_status(token: token) }

    before do
      allow_any_instance_of(Uploadcare::VideoConverterClient).to receive(:status)
        .with(token: token, request_options: {})
        .and_return(response_body)
    end

    it 'returns an instance of VideoConverter' do
      expect(subject).to be_a(Uploadcare::VideoConverter)
    end
