...
RSpec.describe Uploadcare::WebhookClient do
  ...
  describe '#create_webhook' do
    ...
    it 'creates a new webhook with options hash' do
      response = webhook_client.create_webhook(options: options)
      expect(response.success).to eq(response_body)
    end
    ...
    it 'uses default values when options are missing' do
      ...
      response = webhook_client.create_webhook(options: { target_url: target_url })
      expect(response.success).to eq(response_body)
    end
  end
  describe '#update_webhook' do
    ...
    it 'updates the webhook with options hash' do
      response = webhook_client.update_webhook(id: webhook_id, options: payload)
      expect(response.success).to eq(response_body)
    end
    ...
    it 'accepts partial updates' do
      ...
      response = webhook_client.update_webhook(id: webhook_id, options: partial_payload)
      expect(response.success).to eq(response_body)
    end
  end
  ...
  describe '#delete_webhook' do
    ...
    it 'deletes the webhook successfully' do
      result = subject.delete_webhook(target_url: target_url)
      expect(result.failure?).to be false
    end

    it 'sends target_url in request body' do
      result = subject.delete_webhook(target_url: target_url)
      expect([nil, '']).to include(result.success) # API may return empty string or nil
    end
    ...
    it 'handles various URL formats' do
      ...
      urls.each do |url|
        ...
        result = subject.delete_webhook(target_url: url)
        expect(result.failure?).to be false
      end
    end
  end
