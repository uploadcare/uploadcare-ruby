...
RSpec.describe Uploadcare::AddonsClient do
  ...
  describe '#aws_rekognition_detect_labels' do
    ...
    subject(:result) { client.aws_rekognition_detect_labels(uuid: uuid) }
    ...
    context 'when the request is successful' do
      ...
      it 'returns a valid request ID' do
        expect(result.success['request_id']).to eq('8db3c8b4-2dea-4146-bcdb-63387e2b33c1')
      end
    end
  ...
  describe '#aws_rekognition_detect_labels_status' do
    ...
    subject(:result) { client.aws_rekognition_detect_labels_status(request_id: request_id) }
    ...
    context 'when the request is successful' do
      ...
      it 'returns the correct status' do
        expect(result.success['status']).to eq('in_progress')
      end
    end
  ...
  describe '#aws_rekognition_detect_moderation_labels' do
    ...
    it 'returns the request ID' do
      response = client.aws_rekognition_detect_moderation_labels(uuid: uuid)
      expect(response.success).to eq(response_body)
    end

    it 'returns a failure when the request fails' do
      stub_request(:post, 'https://api.uploadcare.com/addons/aws_rekognition_detect_moderation_labels/execute/')
        .with(body: { target: uuid }.to_json)
        .to_return(
          status: 400,
          body: { 'detail' => 'Bad Request' }.to_json,
          headers: { 'Content-Type' => 'application/json' }
      ...
      expect(response.error.message).to eq('Bad Request')
    end
  end
  describe '#aws_rekognition_detect_moderation_labels_status' do
    ...
    it 'returns the status' do
      response = client.aws_rekognition_detect_moderation_labels_status(request_id: request_id)
      expect(response.success).to eq(response_body)
    end
  ...
  describe '#uc_clamav_virus_scan' do
    ...
    it 'returns the request ID' do
      response = client.uc_clamav_virus_scan(uuid: uuid, params: params)
      expect(response.success).to eq(response_body)
    end
  ...
  describe '#uc_clamav_virus_scan_status' do
    ...
    it 'returns the status' do
      response = client.uc_clamav_virus_scan_status(request_id: request_id)
      expect(response.success).to eq(response_body)
    end
  ...
  describe '#remove_bg' do
    ...
    it 'returns the request_id' do
      response = client.remove_bg(uuid: uuid, params: params)
      expect(response.success).to eq(response_body)
    end
  ...
  describe '#remove_bg_status' do
    ...
    it 'returns the status and result' do
      response = client.remove_bg_status(request_id: request_id)
      expect(response.success).to eq(response_body)
    end
