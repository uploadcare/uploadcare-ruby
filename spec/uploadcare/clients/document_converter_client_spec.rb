...
RSpec.describe Uploadcare::DocumentConverterClient do
  ...
  describe '#convert_document' do
    ...
    subject(:result) { client.convert_document(paths: paths, options: options) }
    ...
    context 'when the request returns an error' do
      before do
        stub_request(:post, full_url)
          .to_return(status: 400, body: { 'detail' => 'Bad Request' }.to_json,
                     headers: { 'Content-Type' => 'application/json' })
      end

      it 'returns a failure' do
        response = client.convert_document(paths: paths, options: options)
  ...
  describe '#status' do
    ...
    context 'when the request returns an error' do
      ...
      it 'returns a failure' do
        response = client.status(token: token)
        expect(response.failure?).to be(true)
        expect(response.error).to be_a(Uploadcare::Exception::RequestError)
