    ...
    RSpec.describe Group do
      ...
      describe 'cdn_url' do
        ...
        before do
          ...
          allow(Uploadcare.config).to receive(:effective_cdn_base).and_call_original
        end

        it 'generates CDN URL using effective_cdn_base config' do
          allow(Uploadcare.config).to receive(:effective_cdn_base).and_return(-> { 'https://example.ucarecdn.com' })
        ...
        it 'handles different CDN base configurations' do
          ...
          test_cases.each do |test_case|
            allow(Uploadcare.config).to receive(:effective_cdn_base).and_return(-> { test_case[:base] })
        ...
        it 'works with group initialized from cdn_url' do
          ...
          allow(Uploadcare.config).to receive(:effective_cdn_base).and_return(-> { 'https://test.cdn.com' })

          result = cdn_url_group.cdn_url
          expect(result).to eq("https://test.cdn.com#{test_group_id}/")
        end

        it 'calls effective_cdn_base each time for dynamic config updates' do
          allow(Uploadcare.config).to receive(:effective_cdn_base).and_return(-> { 'https://first.cdn.com' })
          first_call = group.cdn_url

          allow(Uploadcare.config).to receive(:effective_cdn_base).and_return(-> { 'https://second.cdn.com' })
        ...
        it 'works with subdomains when enabled' do
          ...
          allow(Uploadcare.config).to receive(:effective_cdn_base).and_return(-> { 'https://abc123def.ucarecdn.com' })
        ...
        it 'handles custom CNAME domains' do
          allow(Uploadcare.config).to receive(:effective_cdn_base).and_return(-> { 'https://my-custom-domain.com' })
        ...
        context 'integration with real config' do
          it 'generates valid CDN URL with default config' do
            # Don't mock effective_cdn_base to test real integration
            result = group.cdn_url
      ...
      describe 'file_cdn_urls' do
        ...
        before do
          allow(Uploadcare.config).to receive(:effective_cdn_base).and_return(-> { 'https://ucarecdn.com/' })
        end
        ...
        it 'works with different CDN base configurations' do
          allow(Uploadcare.config).to receive(:effective_cdn_base).and_return(-> { 'https://subdomain.ucarecdn.com/' })
          files_collection = double('files_collection', count: 2)
          allow(group).to receive(:files).and_return(files_collection)
